...
Works for the genbank files generated by Ensembl_bacteria.
	''')
...
def parse_genbank(gbk, locus):
	for seq in SeqIO.parse(open(gbk,'r'),"genbank"):
		...
		for feat in seq.features:
			if feat.type == "CDS":
				if feat.qualifiers["protein_id"][0] == locus:
...
def plot(seq, span, coords, name):
	...
	# toggle this block to add GC content info
		...
		if feat.type == "CDS":
			if int(feat.location.start) > (coords[0]-(span/2)) and int(feat.location.end) < (coords[1]+(span/2)):
				gd_feature_set.add_feature(feat, sigil="BIGARROW", arrowshaft_height=1, arrowhead_length=.4,color=cl[count % len(cl)], \
					label=True,name=feat.qualifiers['protein_id'][0],label_strand=-1,label_size = 8,label_position="middle", label_angle=90, \
					border=colors.black)
				count += 1
		elif feat.type == "gene":
			if int(feat.location.start) > (coords[0]-(span/2)) and int(feat.location.end) < (coords[1]+(span/2)):
				gd_feature_set.add_feature(feat, sigil="BIGARROW", arrowshaft_height=1, arrowhead_length=.4,color=cl[count % len(cl)], \
					label=True,name=feat.qualifiers['note'][0],label_strand=1,label_size = 8,label_position="middle", label_angle=20, \
					border=colors.black)
...
def main():
	...
	plot(contigseq, span, coords,locus+".pdf")
	# get_GC_content(contigseq)
